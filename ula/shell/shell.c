...
/*
 * Here is a note.
 * If you find bugs, and you want to add it to 'bugs' command.
...
void shell_ver(int argc){
    ...
    console_write("RainyOS Alpha Version 0.58 Build 17\n");
    console_write("RainyShell Alpha Version 0.32\n\n");
}
...
void shell_help(int argc){
    ...
    console_write("_______________________________________________________________________\n");
    ...
    console_write("|REBOOT           |No argument. Reboot your computer                  |\n");
    console_write("|TESTAPI          |No argument. Test RainyOS APIs                     |\n");
    console_write("|CALM             |No argument. Send HLT command to CPU               |\n");
    console_write("|*SHUTDOWN        |No argument. Shutdown your computer                |\n");
    console_write("|---------------------------------------------------------------------|\n");
    console_write("Note: If a command is unavailable, it will be marked with '*'.\n\n");
}
...
void shell_testapi(int argc){
    if (argc != 1){
        console_write_color("Command unsupport more than 1 argument\n", rc_black, rc_red);
    }
    api_print("API_print function OK\n");
    api_printc("API_printc function OK\n", rc_black, rc_green);
    api_datetime();
    api_print("\n");
    //char testapiinput = api_input("API_input function OK\n\n");
    //printk(testapiinput);
}

void shell_calm(int argc){
    if (argc != 1){
        console_write_color("Command unsupport more than 1 argument\n", rc_black, rc_red);
    }
    asm volatile ("hlt");
    console_write("Sent HLT command to CPU\n\n");
}

void shell_shutdown(int argc) {
    if (argc != 1){
        console_write_color("Command unsupport more than 1 argument\n", rc_black, rc_red);
    }
    //acpi_shutdown();
    console_write_color("Unavailable command.", rc_black, rc_red);
}
